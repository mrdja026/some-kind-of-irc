...
- [x] 1.4 Define operational overhead shifts (K3s, NGINX Ingress, Redis, PostgreSQL, Helm, CI/CD, monitoring)
...
- [x] 2.2 Deploy NGINX Ingress Controller and validate ingress routing — `k8s/scripts/03-install-nginx-ingress.sh`
- [x] 2.3 Deploy Redis and PostgreSQL pods for local dev — `k8s/scripts/04-deploy-redis-postgres.sh`
- [x] 2.4 Deploy monolith container and Hello World FastAPI service — `k8s/scripts/05-deploy-monolith-hello.sh`
- [x] 2.5 Configure ingress routes for /auth and /ai to validate cutover behavior — `k8s/scripts/06-configure-ingress.sh`
- [x] 2.6 Seed default users (admina/guest) in K3s monolith ✅ **COMPLETED**
- [ ] 2.7 Deploy audit-logger throwaway microservice for strangle pattern testing — **TD** (deferred to Helm deployment only)
