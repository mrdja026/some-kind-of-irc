...
#### 2. Deployment (Helm + kubectl)

| Task                  | Current                 | Target                             |
| --------------------- | ----------------------- | ---------------------------------- |
| Deploy new version    | `docker compose up -d`  | `kubectl apply` + rollout restart  |
| Rollback              | Manual `docker compose` | `kubectl rollout undo`             |
| Config changes        | Edit `.env.local`       | Update ConfigMap/Helm values + apply |
| Multi-env support     | Not supported           | Helm values per environment        |
| Deployment visibility | Terminal output         | `kubectl rollout status`           |

**Net change**: +Medium initially, but **reduces** long-term deployment friction with repeatable manifests.
...
| Deploy       | Manual                 | CI applies manifests to K8s             |
...
The migration adds **medium operational overhead** primarily from K3s cluster management, PostgreSQL/Redis provisioning, and Helm/K8s setup. However, this overhead is front-loaded (one-time setup) and pays off through:
