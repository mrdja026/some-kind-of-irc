...
def create_test_channels():
    ...
    for channel_data in channels_to_create:
        ...
        existing_channel = (
            db.query(Channel).filter(Channel.name == channel_data["name"]).first()
        )
        if existing_channel:
            print(
                f"Channel {channel_data['name']} already exists (ID: {existing_channel.id})"
            )
            existing_count += 1
            continue

        # Create a new channel
        new_channel = Channel(name=channel_data["name"], type=channel_data["type"])
        ...
        print(
            f"Channel {channel_data['name']} created successfully. ID: {new_channel.id}"
        )
        created_count += 1
