...
def _user_has_local_qa_access(user: User) -> bool:
    return _user_has_ai_access(user)
...
async def create_channel(
    ...
    if channel.type == "public" and not channel.name.startswith("#"):
        raise HTTPException(
            status_code=400, detail="Public channel name must start with #"
        )
    ...
    if channel.is_data_processor and not settings.data_processor_enabled:
        raise HTTPException(
            status_code=400, detail="Data processor feature is not enabled"
        )
    ...
    new_channel = Channel(
        name=channel.name,
        type=channel.type,
        is_data_processor=channel.is_data_processor,
    )
    db.add(new_channel)
    db.commit()
        ...
        try:
            ...
            if other_user:
                other_membership = Membership(
                    user_id=_as_int(other_user.id), channel_id=new_channel_id
                )
...
async def get_channels(
    ...
    private_channels = (
        db.query(Channel)
        .filter(Channel.id.in_(private_channel_ids), Channel.type == "private")
        .all()
...
async def get_direct_messages(
    ...
    return (
        db.query(Channel)
        .filter(Channel.id.in_(channel_ids), Channel.type == "private")
        .all()
    )
...
async def create_direct_message_channel(
    ...
    if user_id == current_user.id:
        ...
        existing_self_dm = (
            db.query(Channel).filter(Channel.name == self_dm_name).first()
        )
        if existing_self_dm:
            existing_membership = (
                db.query(Membership)
            ...
            if not existing_membership:
                db.add(
                    Membership(user_id=current_user.id, channel_id=existing_self_dm.id)
                )
                db.commit()
    ...
    existing_channel = (
        db.query(Channel)
        .filter((Channel.name == dm_name1) | (Channel.name == dm_name2))
        .first()
    )
    if existing_channel:
        ...
        if existing_membership:
            ...
            membership = Membership(
                user_id=current_user.id, channel_id=existing_channel.id
            )
            db.add(membership)
            db.commit()
            # Update WebSocket manager for both users
...
async def get_channel_members(
    ...
    query = (
        ...
        .filter(Membership.channel_id == channel_id)
    )
    ...
    def sort_key(user: User):
        ...
        display_name = (
            display_name_value.lower() if display_name_value else "zzzzzzzzzz"
        )
...
async def add_member_to_channel(
    ...
    user_to_add = (
        db.query(User).filter(User.username == member_request.username).first()
    )
    if not user_to_add:
    ...
    existing_membership = (
        db.query(Membership)
        .filter(
            Membership.user_id == user_to_add_id, Membership.channel_id == channel_id
        )
        .first()
    )
    if existing_membership:
    ...
    return {
        "message": f"Added {_as_str(user_to_add.username)} to channel {channel_name}"
    }
